...
isolated function nativeWaitForExit(Process process) returns int | Error = @java:Method {
    name: "waitForExit",
    'class: "org.ballerinalang.stdlib.transaction.testutils.nativeimpl.WaitForExit"
} external;

isolated function nativeExitCode(Process process) returns int | Error = @java:Method {
    name: "exitCode",
    'class: "org.ballerinalang.stdlib.transaction.testutils.nativeimpl.ExitCode"
} external;

isolated function nativeStderr(Process process) returns io:ReadableByteChannel = @java:Method {
    name: "stderr",
    'class: "org.ballerinalang.stdlib.transaction.testutils.nativeimpl.Stderr"
} external;
